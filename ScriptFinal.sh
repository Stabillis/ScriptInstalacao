...
java -version
if [ $? -eq 0 ]; then
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) VocÃª jÃ¡ tem o Java instalado!"
else
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Opa! NÃ£o identifiquei nenhuma versÃ£o do Java instalada, mas sem problemas, irei resolver isso agora!"
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Confirme para mim se realmente deseja instalar o Java (S/N)?"
    read -r inst
    if [ "$inst" == "S" ]; then
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Ok! VocÃª escolheu instalar o Java ;D"
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Adicionando o repositÃ³rio!"
        sleep 2
        sudo add-apt-repository ppa:webupd8team/java -y
        clear
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Atualizando! Quase lÃ¡."
        sleep 2
        sudo apt update -y
        clear
        ...
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) VocÃª optou por nÃ£o instalar o Java por enquanto, atÃ© a prÃ³xima entÃ£o!"
    fi
fi
...
if [ $? -eq 0 ]; then
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) VocÃª jÃ¡ tem o Docker instalado!"
else
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Opa! NÃ£o identifiquei nenhuma versÃ£o do Docker instalada, mas sem problemas, irei resolver isso agora!"
    echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Podemos instalar o Docker em sua mÃ¡quina (S/N)?"
    read -r inst
    if [ "$inst" == "S" ]; then
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Ok! VocÃª escolheu instalar o Docker ;D"
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Instalando o Docker em sua mÃ¡quina!"
        sleep 2
        sudo apt install docker.io -y
        echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Ativando o Docker na mÃ¡quina"
        sleep 3
        sudo systemctl start docker
        sleep 7
	echo "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) Habilitando o serviÃ§o"
...
if [ "$(sudo docker ps -aqf name=Stabillis)" ]; then
        echo -e "$(tput setaf 10)[Assistente - Stabillis]:$(tput setaf 7) VocÃª jÃ¡ possui o banco de dados em seu sistema! Vamos inicializar o container"
        sleep 2
